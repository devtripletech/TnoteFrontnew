...
const url = "http://54.162.218.30:3000/"
  ...
  return getToken().then(async (token) => {
    try {
      const res = await fetch(
        `http://54.162.218.30:3000/horasprojetoindex/${user?.email}`,
        {
          method: "GET",
          headers: {
...
export const addAttendanceAction = async (
  ...
  return getToken().then(async (token) => {
    const res = await fetch(`http://54.162.218.30:3000/horasprojeto`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
...
export async function editAttendanceAction(
  ...
  const res = await fetch(
    `http://54.162.218.30:3000/horasprojeto/${input.ID_lancamento}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
export async function duplicateAttendanceAction(id: number) {
  return getToken().then(async (token) => {
    const res = await fetch(`http://54.162.218.30:3000/horasprojetodup/${id}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
