...
export const getClientsAction = async (): Promise<Client[]> => {
  return getToken().then(async (token) => {
    try {
      const res = await fetch(`http://54.162.218.30:3000/cliente`, {
        method: "GET",
        headers: {
...
export const getClientByIdAction = async (
  ...
  return getToken().then(async (token) => {
    try {
      const res = await fetch(`http://54.162.218.30:3000/cliente/${clientId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  return getToken().then(async (token) => {
    try {
      const res = await fetch(
        `http://54.162.218.30:3000/cliente/${input.ID_cliente}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
